            ...
            async with client.stream("POST", url, json=PAYLOAD) as resp:
                ...
                async for raw_line in resp.aiter_lines():
                    ...
                    if event_type == "token":
                        ...
                        print(
                            f"[{label}] chunk #{len(chunks):>3} | t={elapsed:.3f}s | len={len(content):>5} chars | {repr(content[:50])}"
                        )
                        ...
                        print(
                            f"[{label}] citations received at {elapsed:.3f}s ({len(parsed.get('citations', []))} citations)"
                        )
...
def summarize(label: str, r: dict):
    ...
    if r["num_chunks"] > 1:
        ...
        print(
            f"  Chunk sizes:         min={min(r['chunk_sizes'])} max={max(r['chunk_sizes'])} avg={sum(r['chunk_sizes']) // len(r['chunk_sizes'])}"
        )
...
async def main():
    ...
    if lc > 1 and pc == 1:
        ...
        print(
            "     Fix: ensure proxy passes 'X-Accel-Buffering: no' and 'Cache-Control: no-cache'."
        )
    elif lc == 1 and pc == 1:
        print(
            "  🚨 BOTH are buffering. The LLM itself is not streaming (thinking model or OpenRouter issue)."
        )
    elif lc > 1 and pc > 1:
